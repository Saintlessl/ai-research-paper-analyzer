export interface User {
    ...
    role?: 'researcher' | 'reviewer' | 'admin' | null;
    roles?: Array<'researcher' | 'reviewer' | 'admin' | { name: string }>;
}
export interface Capabilities { upload_papers: boolean; review_papers: boolean; manage_system: boolean; }
...
export interface Paper { id: number; title: string; abstract?: string | null; status?: string | null; publication_year?: number | null; journal?: string | null; doi?: string | null; keywords?: string[] | null; authors?: Author[]; created_at?: string; updated_at?: string; progress?: number | null; error_message?: string | null; overall_score?: number | null; failed_job_id?: number | null; }
...
type SharedPageProps<T extends Record<string, unknown>, AuthenticatedUser extends User | null> = T & { auth: { user: AuthenticatedUser; capabilities: Capabilities; impersonating?: boolean; is_actual_super_admin?: boolean; active_role_override?: string }; flash?: { success?: string; error?: string; info?: string }; };
export type PageProps<T extends Record<string, unknown> = Record<string, unknown>> = SharedPageProps<T, User | null>;
export type AuthenticatedPageProps<T extends Record<string, unknown> = Record<string, unknown>> = SharedPageProps<T, User>;
